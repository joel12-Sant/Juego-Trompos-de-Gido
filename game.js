...
(function(){
  ...
  const AudioSys = (()=>{
    let ac, musicGain, sfxGain, musicInterval;
    function init(){ if(!ac){ ac = new (window.AudioContext||window.webkitAudioContext)(); musicGain = ac.createGain(); sfxGain = ac.createGain(); musicGain.gain.value=.05; sfxGain.gain.value=.25; musicGain.connect(ac.destination); sfxGain.connect(ac.destination);} }
    function beep(freq=440, dur=.08){ if(!ac) return; const o=ac.createOscillator(); const g=ac.createGain(); o.frequency.value=freq; o.type='square'; o.connect(g); g.connect(sfxGain); g.gain.setValueAtTime(.0001, ac.currentTime); g.gain.exponentialRampToValueAtTime(.4, ac.currentTime+.01); g.gain.exponentialRampToValueAtTime(.0001, ac.currentTime+dur); o.start(); o.stop(ac.currentTime+dur); }
    function hit(){ beep(180,.12) }
    function pickup(){ beep(720,.08) }
    function startMusic(){ if(!ac) return; stopMusic();
      musicInterval = setInterval(()=>{ const o=ac.createOscillator(); const g=ac.createGain(); const base=[220,247,262,294,330,349,392][Math.floor(Math.random()*7)]; o.frequency.value = base * (Math.random()<.3?1.5:1); o.type='sine'; o.connect(g); g.connect(musicGain); const t=ac.currentTime; g.gain.setValueAtTime(.0001,t); g.gain.linearRampToValueAtTime(.12,t+.2); g.gain.exponentialRampToValueAtTime(.0001,t+.8); o.start(); o.stop(t+.85); }, 900);
    }
    function stopMusic(){ if(musicInterval) clearInterval(musicInterval); }
    return { init, beep, hit, pickup, startMusic, stopMusic };
  })();
  ...
  function start(){ S.running=true; S.paused=false; S.t0=performance.now(); S.last=S.t0; ui.overlay.style.display='none'; ui.btn.textContent='Pausa'; AudioSys.init(); AudioSys.startMusic(); }
  function togglePause(){ if(!S.running) { start(); return; } S.paused=!S.paused; ui.btn.textContent = S.paused? 'Reanudar' : 'Pausa'; if(S.paused){ AudioSys.stopMusic(); ui.overlay.style.display='grid'; } else { AudioSys.startMusic(); ui.overlay.style.display='none'; S.last = performance.now(); } }
  function gameOver(){
